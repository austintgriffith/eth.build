...
function addHelpers(obj) {
    ...
    obj.prototype.parseContract = function () {
      ...
      for (let i in this.abi) {
        if (this.abi[i].type == "function") {
          ...
          if (!this.types[this.abi[i].name] && this.abi[i].constant)
            this.types[this.abi[i].name] = "view"
        }
      }
      ...
      try {
        ...
        for (let name in this.functions) {
          ...
          outputs.push([
            name + "()",
            this.types[name] == "view" ? "contractCall" : "contractFunction",
            null,
          ])
          let linksArray = []
        ...
        if (this.types[name] == "view") {
          ...
          this.setOutputData(index++, {
            ...
            function: async args => {
              ...
              let thisContract = new this.web3.eth.Contract(
                this.abi,
                this.address
              )
  ...
  obj.prototype.render = function (reactElement) {
    ...
    if (this.graph && this.graph.canvas && this.graph.canvas.ds) {
      ReactDOM.render(
        <div
          key={this.id + "_react_key"}
          style={{
            position: "absolute",
            left:
              (20 + this.pos[0] + this.graph.canvas.ds.offset[0]) *
              this.graph.canvas.ds.scale,
            top:
              (this.pos[1] + this.graph.canvas.ds.offset[1]) *
              this.graph.canvas.ds.scale,
        ...
        >
          <div
            stlye={{ transform: "scale(" + this.graph.canvas.ds.scale + ")" }}
          >
            {reactElement}
          </div>
        </div>,
      ...
      if (canvas) {
        ...
        console.log(
          "no graph.canvas I bet it is sub sub module",
          canvas,
          graph._subgraph_node.graph.canvas
        )
      }
    }
  }
  ...
  obj.prototype.onOutputDblClick = function (index, e) {
    if (typeof obj.prototype.originalOnOutputDblClick == "function")
      obj.prototype.originalOnOutputDblClick()
    ...
    if (this.outputs[index].type == -1) {
      ...
      if (
        this.outputs[index] &&
        this.outputs[index]._data &&
        typeof this.outputs[index]._data.substr == "function" &&
        this.outputs[index]._data.length == 42 &&
        this.outputs[index]._data.substr(0, 2) == "0x"
      ) {
        var node_watch = globalLiteGraphJS.LiteGraph.createNode(
          "Display/Address"
        )
        node_watch.pos = [e.canvasX + 90, e.canvasY - 25]
        this.graph.add(node_watch)
        ...
        this.outputs[index]._data &&
        typeof this.outputs[index]._data.substr == "function" &&
        this.outputs[index]._data.length > 96
      ) {
        var node_watch = globalLiteGraphJS.LiteGraph.createNode(
          "Display/TextArea"
        )
        ...
        this.outputs[index] &&
        this.outputs[index]._data &&
        typeof this.outputs[index]._data == "object"
      ) {
        var node_object = globalLiteGraphJS.LiteGraph.createNode(
          "Object/Object"
        )
        ...
        try {
          ...
          if (value > 0.0001 && value < 1000000000) {
            ...
            var node_from_wei = globalLiteGraphJS.LiteGraph.createNode(
              "Utils/From Wei"
            )
  ...
  obj.prototype.onInputDblClick = function (index, e) {
    if (typeof obj.prototype.originalOnInputDblClick == "function")
      obj.prototype.originalOnInputDblClick()
    if (this.inputs[index].type == -1) {
      if (this.type == "Input/Button") {
        ...
        var node_button = globalLiteGraphJS.LiteGraph.createNode(
          "Control/Timer"
        )
      ...
      this.inputs[index].type.indexOf("string") == 0
    ) {
      ...
      this.inputs[index].type &&
      this.inputs[index].type.indexOf("object") == 0
    ) {
      ...
      this.inputs[index].type &&
      this.inputs[index].type.indexOf("number") == 0
    ) {
  ...
  obj.prototype.onDblClick = function (e, location, canvas) {
    ...
    if (typeof this.originalOnDblClick == "function")
      this.originalOnDblClick(e, location, canvas)
  }

  obj.prototype.destory = function () {
    let possibleReactElement = document.getElementById(
      this.id + "_react_element"
    )
  ...
  obj.prototype.onHideNode = function () {
    if (typeof obj.prototype.originalonHide == "function")
      obj.prototype.originalonHide()
    this.destory()
  }
  ...
  obj.prototype.onRemoved = function (e) {
    if (typeof obj.prototype.originalOnRemoved == "function")
      obj.prototype.originalOnRemoved()
    this.destory()
  }
...
const hexColor = hex => {
  ...
  return [
    "rgba(" + rgbVale.red + "," + rgbVale.green + "," + rgbVale.blue + ")",
    "rgba(" + rgbVale.red + "," + rgbVale.green + "," + rgbVale.blue + ",0.25)",
  ]
}
...
export const loadCustomNode = LiteGraphJS => {
  ...
  for (let n in global.customNodes) {
    addNodes(
      ...
      ...hexColor(global.customNodes[n].color)
    )
  }
  console.log(
    "HOOK INTO CLOSE AND OPEN OF SUBGRAPH TO DESTROY NODES?",
    LiteGraphJS
  )
